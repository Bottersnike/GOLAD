...
function drawAll() {
    ...
    xOff = (canvas.width - ((GRID_WIDTH * tileSize) + TILE_PADDING * (GRID_WIDTH + 1))) / 2;
    yOff = (canvas.height - ((GRID_HEIGHT * tileSize) + TILE_PADDING * (GRID_HEIGHT + 1))) / 2;
...
function redrawTile(x, y) {
    ctx.fillStyle = BLACK;
    ctx.fillRect(xOff + x * (tileSize + TILE_PADDING) + TILE_PADDING / 2,
        yOff + y * (tileSize + TILE_PADDING) + TILE_PADDING / 2,
        tileSize + (TILE_PADDING),
        tileSize + (TILE_PADDING));
    ...
    roundRect(ctx, xOff + x * (tileSize + TILE_PADDING) + TILE_PADDING,
        yOff + y * (tileSize + TILE_PADDING) + TILE_PADDING,
        tileSize,
        tileSize, Math.floor(tileSize / 10), true, false
    );
    ...
    roundRect(ctx, xOff + x * (tileSize + TILE_PADDING) + TILE_PADDING,
        yOff + y * (tileSize + TILE_PADDING) + 9 * tileSize / 10 + TILE_PADDING,
        tileSize,
        tileSize / 10, Math.floor(tileSize / 10), true, false
    );
    /*ctx.fillRect(xOff + x * (tileSize + TILE_PADDING),
        yOff + y * (tileSize + TILE_PADDING),
        tileSize,
        tileSize
    );*/
    ...
    if (FANCY_MIDDLE)
        ...
        radius = Math.floor(tileSize / 10);

    roundRect(ctx, xOff + x * (tileSize + TILE_PADDING) + tileSize / 3 + TILE_PADDING,
        yOff + y * (tileSize + TILE_PADDING) + tileSize / 3 + TILE_PADDING,
        tileSize / 3,
        tileSize / 3, radius, true, false);

    /*ctx.fillRect(xOff + x * (tileSize + TILE_PADDING) + tileSize / 3,
        yOff + y * (tileSize + TILE_PADDING) + tileSize / 3,
        tileSize / 3,
        tileSize / 3
    );*/
                ...
                if (event.offsetX > rect[0] && event.offsetX < rect[0] + rect[2]) {
                    if (event.offsetY > rect[1] && event.offsetY < rect[1] + rect[3]) {
                        /*if (gridTiles[x][y].currentState == 0) {
                            ...
                            refreshTile(x, y);
                        } else if (gridTiles[x][y].currentState == currentPlayer) {
